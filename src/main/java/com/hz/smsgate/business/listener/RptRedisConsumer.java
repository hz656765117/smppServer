...
//				smppSession.sendRequestPdu(deliverSm, 10000, true);
                ...
                if (smppSession.getConfiguration().getSystemId() != "CM0001" && smppSession.getConfiguration().getSystemId() != "CM0002") {
                    try {
                        smppSession.sendRequestPdu(deliverSm, 10000, true);
                    } catch (SmppTimeoutException ste) {
                        LOGGER.error("{}-systemid:{},mbl:{},msgid:{},短信状态报告转发异常1", Thread.currentThread().getName(), smppSession.getConfiguration().getSystemId(), deliverSm.getSourceAddress().getAddress(),preMsgId, ste);
                        Thread.sleep(3000);
                    }
